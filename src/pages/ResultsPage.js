...
const format = (n, unit = "") =>
  n != null && !isNaN(n) ? `${Number(n).toFixed(0)}${unit}` : "—";
...
function getPrincipalInsight({ principalKey, formData, processedData }) {
  ...
  const bank = {
    obesidad: {
      ...
      action:
        "Apunta a -4–6 cm de cintura en 8–12 semanas con fuerza 2×/semana y 1/2 plato de verduras.",
      tips: [
        "Prioriza proteína magra y legumbres",
        "Evita bebidas azucaradas",
        "Caminatas post-comida 10–15 min",
      ],
    },
    hta: {
      title: "Hipertensión",
      ...
      action:
        "Reduce sodio y ultraprocesados; controla PA 2×/semana y sigue indicaciones médicas.",
      tips: [
        "≥150 min/semana actividad",
        "Lee etiquetas: <1 g sodio/día",
        "Evita AINES prolongados",
      ],
    },
    dieta_baja: {
      ...
      youVsRef: {
        labelYou: `ICD ${format(icd)}`,
        labelRef: "ICD ≥ 2",
        ratio: icd && icd < 2 ? icd / 2 : 1,
      },
      action:
        "Suma +2 porciones/día de verduras/frutas y cambia cereales refinados por integrales.",
    ...
    tabaquismo: {
      ...
      youVsRef: {
        labelYou: `ICT ${format(ict)}`,
        labelRef: "ICT 0",
        ratio: ict ? Math.min(1, 0.3 + ict / 10) : 0.3,
      },
      action:
        "Define fecha de cesación y apoyo (farmacológico/conductual). Mejoras metabólicas en semanas.",
    ...
    salud_mental: {
      ...
      youVsRef: {
        labelYou: `IBM ${format(ibm)}`,
        labelRef: "IB ≤ 6",
        ratio: ibm ? Math.min(1, ibm / 6) : 0.6,
      },
      action:
        "Rutina 10 min/día: respiración 4-7-8 + paseo breve; cuida higiene del sueño.",
    ...
    edad: {
      ...
      youVsRef: {
        labelYou: `${format(edad)} años`,
        labelRef: "Fuerza y NEAT altos",
        ratio: 1,
      },
      action:
        "Entrenamiento de fuerza 2×/semana + caminatas diarias; prioriza proteína de calidad.",
  ...
  return (
    bank[mapKey] || {
      ...
      action:
        "Mantén los básicos: verduras 1/2 plato, 150 min/semana y sueño 7–8 h.",
 ...
 *  IMPORTANTE: estas rutas esperan que las imágenes estén en /public/img
 */
  ...
  try {
    ...
    if (parseFloat(formData.QS102) === 1) {
      factors.push({
        ...
        icon: "🩺",
        description:
          "El antecedente de hipertensión se relaciona con resistencia a la insulina y mayor riesgo cardiovascular.",
        priority: 1,
      });
    }
    ...
    if (processedData.ICT > 5) {
      factors.push({
        ...
        icon: "🚬",
        description:
          "Exposición acumulada al tabaco que favorece procesos inflamatorios y resistencia a la insulina.",
        priority: 2,
      });
    }
    if (processedData.IBM >= 10) {
      factors.push({
        ...
        icon: "🧠",
        description:
          "Estrés o síntomas depresivos pueden influir en el metabolismo y el autocuidado.",
        priority: 3,
      });
    }
  } catch (e) {
...
const variablesExplicadas = [
  {
    ...
    detalle:
      "Evalúa frutas, verduras y balance nutricional. Mejores puntajes reducen ~30% el riesgo.",
    icon: "🥗",
  },
  {
    nombre: "Proxy de Síndrome Metabólico (PSM)",
    detalle:
      "Combina obesidad abdominal e hipertensión: fuerte predictor clínico de diabetes.",
    icon: "🧬",
  },
  {
    nombre: "Índice de Bienestar Mental (IBM)",
    detalle:
      "Mide síntomas depresivos/estrés, factor que incide en la regulación metabólica.",
    icon: "🧘",
  },
  {
    nombre: "Índice de Carga Tabáquica (ICT)",
    detalle: "Cuantifica exposición acumulada al tabaco.",
    icon: "🚭",
  },
  {
    nombre: "Indicador de Consumo de Alcohol de Riesgo (ICAR)",
    detalle: "Altos consumos se asocian a dislipidemia y resistencia.",
    icon: "🍷",
  },
];
...
const Sparkline = ({ data = [], level = "Bajo" }) => {
  ...
  const ys = data.map(
    (d) => (h - pad) - (Math.min(100, Math.max(0, d.p)) / 100) * (h - pad * 2)
  );
  ...
  // Mostrar máximo 5 etiquetas legibles en el eje X
  const maxLabels = 5;
  ...
  return (
    <div className="graph-box">
      <svg viewBox={`0 0 ${w} ${h}`} className="hist-chart" role="img" aria-label="Historial de riesgo">
        ...
        {/* Eje X (solo algunas fechas) */}
        {days.map((d, i) => {
          if (i % step !== 0 && i !== days.length - 1) return null;
          return <text key={i} x={xs[i] - 10} y={h - pad + 16} className="axis-x">{d}</text>;
        })}
        ...
        <g transform={`translate(${Math.min(lastX - 36, w - 86)}, ${Math.max(lastY - 40, 10)})`}>
          ...
          <text x="40" y="17" textAnchor="middle" fill="#fff" fontWeight="700" fontSize="12">
            {LCOL.emoji} {last.p}%
          </text>
        </g>
      </svg>
...
export default function ResultsPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text(
        "Reporte informativo. No reemplaza la consulta, diagnóstico ni tratamiento médico profesional.",
        pdfWidth / 2,
        footerY + 7,
  ...
  const visuals = useMemo(
    () => getPersonalizedVisuals({ level, factors: topFactors, formData }),
    [level, topFactors, formData]
  );
  const plan = useMemo(
    () => getActionPlan({ level, factors: topFactors }),
    [level, topFactors]
  );
    ...
    <div className="results-container" style={accentStyle}>
      ...
      {invalidData ? (
        <div className="results-card error-card anim-in">
          ...
          <button onClick={() => navigate("/evaluacion")} className="primary-button-green">
        ...
        <div className={`results-card ${cardTone}-tone anim-in`} id="reporte" aria-live="polite">
          {showExitWarn && !bannerClosed && (
            <div className="exit-banner" role="alert">
              <div>
                ⚠️ <strong>Advertencia:</strong> el mini historial se guarda sólo en este
                navegador y puede <u>perderse al cerrar</u>. Te recomendamos{" "}
                <strong>descargar el PDF</strong> como guía.
              </div>
              <button className="banner-close" onClick={() => setBannerClosed(true)}>
                Entendido
              </button>
            </div>
          )}
          ...
          {/* HERO */}
          {/* HERO – ahora dentro de un contenedor blanco */}
{/* HERO – ahora con frase de alerta y etiqueta bajo el anillo */}
<section className="hero-card">
  <header className="hero">
    <div className="hero-left">
      {/* Frase de alerta contextual */}
      <h2 className="results-title">Tus resultados personalizados</h2>
      <p className="hero-alert">
Tu probabilidad de desarrollar diabetes es de <strong>{clampPct(probability)}%</strong>. Con base en tus respuestas, considera estas recomendaciones para reducir tu riesgo a lo largo del tiempo y vuelve a evaluarte cuando desees.</p>
    ...
    <div
      className={`ring-wrap ring-pulse-soft ${level === "Alto" ? "ring-pulse-strong" : ""}`}
      aria-label={`Riesgo ${animatedPct}%`}
    >
      ...
      {/* etiqueta bajo el gráfico */}
      <p className="ring-label">Probabilidad estimada</p>
    </div>
  </header>
</section>
          ...
          {/* INTERPRETACIÓN + CONTEXTO (fusionados) */}
          <section className="detail-card context-card highlight">
            ...
            <div className="micro-kpis">
              <div className="kpi">
                <span>Probabilidad Obtenida</span>
                <strong>{clampPct(probability)}%</strong>
              </div>
              <div className="kpi">
                <span>Plan sugerido</span>
                <strong>{actionsTone}</strong>
              </div>
              <div className="kpi">
                <span>Umbral óptimo en base a tu edad</span>
                <strong>{(OPTIMAL_THRESHOLD * 100).toFixed(1)}%</strong>
              </div>
            </div>
            ...
            {(() => {
              ...
              const left = Math.min(98, Math.max(2, pctVal));
              return (
                <div
                  className="microbar-wrap"
                  role="img"
                  aria-label={`Probabilidad ${pctVal}% sobre una escala 0 a 100`}
                >
              ...
              return (
                <div className="insight-grid">
                  <div className="insight-card">
                    <div className="ins-title">💡Importante</div>
                  ...
                  <div className="insight-card">
                    ...
                    <div className="compare">
                      <div className="cmp-labels">
                        <span>
                          Tu valor: <strong>{insight.youVsRef.labelYou}</strong>
                        </span>
                        <span>
                          Referencia: <strong>{insight.youVsRef.labelRef}</strong>
                        </span>
                      </div>
                      <div className="cmp-bar">
                        <div
                          ...
                          style={{
                            width: `${Math.min(
                              100,
                              Math.max(10, (insight.youVsRef.ratio || 0.5) * 100)
                            )}%`,
                          }}
                        />
                      </div>
          ...
          <section className="detail-card action-plan">
            ...
            <div className="advice">Tip: Marca tus logros, puedes volver cuando quieras.</div>
          </section>

          {/* Mini historial – lista izquierda / gráfico derecha */}
          <section className="detail-card">
            <h3>Tu mini historial</h3>
            {history?.length ? (
                  ...
                  {history.map((h, i) => {
                    ...
                    return (
                      <li key={i}>
                        ...
                        <span className="h-pct">
                          {emo} <strong>{val}%</strong>
                        </span>
                      </li>
                    );
                  })}
              ...
              <p>
                Aún no hay registros. Se irán agregando automáticamente cuando veas tus
                resultados.
              </p>
            )}
            <div className="advice">
              Tip: Descarga el PDF tras cada evaluación para llevar un registro fuera del sistema.
            </div>
          </section>
          ...
          <section className="detail-card variables">
            ...
            <ul className="vars-list">
              {variablesExplicadas.map((v, i) => (
                <li key={i}>
                  <span className="var-icon" aria-hidden>
                    {v.icon}
                  </span>
          ...
          <section className="detail-card final-note">
            ...
            <p>
              Esta herramienta utiliza tus respuestas de forma <strong>anónima</strong> y no
              guarda información personal. Los resultados no representan un{" "}
              <strong>diagnóstico médico</strong>, sino una orientación preventiva.
            </p>
            <p>
              🌱 Cada pequeño cambio cuenta. Repite tu evaluación dentro de un año y
              descarga el PDF para comparar tus resultados.
            </p>
          </section>
          ...
          <div className="button-group">
            ...
            <button onClick={() => navigate("/evaluacion")} className="primary-button-green alt">
              Realizar otra evaluación
            </button>
            <Link to="/" className="primary-button-green home-button">
              Volver al inicio
            </Link>
          </div>
        </div>
